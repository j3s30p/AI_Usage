...
OAuth·CLI 값이 새 statusLine 결과를 가리지 않게 합니다.
...
세 모드는 서로 연쇄 실행되지 않습니다. 선택한 실험 모드가 실패했을 때만 statusLine 캐시를
확인합니다. 기본값은 `statusLine 캐시 (권장)`입니다.
...
```text
Claude Code → Scripts/claude-statusline-aiusage.sh
...
- `https://api.anthropic.com/api/oauth/usage` 조회에 실패하거나 Keychain을 조용히 읽을 수 없으면
  statusLine 캐시만 확인합니다. CLI나 로그인 절차는 실행하지 않습니다.
- 사용자가 설정의 `Keychain 연결` 버튼을 직접 눌렀을 때만 macOS 승인 창이 표시될 수 있습니다.
  지속 승인을 원하면 macOS 창에서 `항상 허용`을 선택해야 하며, 앱은 이 동작을 자동 재시도하지
  않습니다.
...
- 매 조회 전에 `claude auth status --json`으로 로그인 상태를 확인합니다.
- 로그인된 경우에만 대화형 `/usage`를 실행합니다.
- AiUsage는 로그아웃 상태에서 `/usage`, 자동 로그인 또는 브라우저 절차를 시작하지 않습니다.
- 인증 확인과 `/usage` 프로세스는 timeout·취소 시 자식 프로세스까지 종료합니다.
- 로그인 확인, CLI 실행 또는 응답 해석이 실패하면 statusLine 캐시만 확인합니다.

외부 Claude CLI가 Keychain 잠금 해제나 접근 승인을 자체적으로 요청하는 상황까지 AiUsage가
차단할 수는 없습니다. Keychain UI 가능성을 완전히 없애려면 기본 statusLine 모드를 사용해야
합니다. `/usage` 또한 안정적인 headless API가 아니므로 Claude CLI 변경에 따라 동작하지 않을 수
있습니다.
...
현재 `0.1.0` 프리뷰와 로컬 개발 빌드는 ad-hoc 서명입니다. Homebrew로 파일을 설치할 수는 있지만
...
공증하면 앱의 코드 정체성과 승인 경험은 안정되지만, 사용자가 처음 누르는 `Keychain 연결` 승인
자체를 없애지는 않습니다.
...
계정 이메일, 세션 ID, 프롬프트, 작업 경로, OAuth 토큰, CLI 원문과 서버 오류 본문은 앱 설정이나
